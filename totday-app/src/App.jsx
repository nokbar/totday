...
// ─── DESIGN SYSTEM · Яркий и свежий ─────────────────────────────────────────
const C = {
  bg:"#FAFAF9",white:"#FFFFFF",dark:"#18181B",ink2:"#52525B",
  gray:"#A1A1AA",muted:"#F4F4F5",line:"#E4E4E7",
  sand:"#E4E4E7",lightGray:"#F4F4F5",champ:"#F9FAFB",champ2:"#F3F4F6",
  taupe:"#A1A1AA",brown:"#18181B",
  rose:"#F43F5E",roseDark:"#E11D48",roseLight:"#FFF1F3",roseMid:"#FDA4AF",roseBg:"#FFF1F3",
  violet:"#7C3AED",violetLight:"#EDE9FE",
  teal:"#059669",tealLight:"#34D399",tealBg:"#D1FAE5",
  gold:"#D97706",goldSoft:"#FCD34D",goldBg:"#FEF3C7",
};
const font="'Plus Jakarta Sans',sans-serif";
const fb="'Plus Jakarta Sans',sans-serif";
const SHADOW="0 1px 3px rgba(0,0,0,.05),0 4px 16px rgba(0,0,0,.06)";
const SHADOW_HOVER="0 4px 6px rgba(0,0,0,.04),0 12px 32px rgba(0,0,0,.10)";
const S={
  app:{fontFamily:fb,background:C.bg,minHeight:"100vh",color:C.dark},
  nav:{background:"rgba(250,250,249,.88)",backdropFilter:"blur(16px) saturate(1.4)",WebkitBackdropFilter:"blur(16px) saturate(1.4)",borderBottom:`1px solid ${C.line}`,padding:"0 28px",display:"flex",alignItems:"center",justifyContent:"space-between",height:66,position:"sticky",top:0,zIndex:200},
  logo:{fontFamily:font,fontSize:23,color:C.dark,fontWeight:800,letterSpacing:"-0.02em",display:"flex",alignItems:"center",gap:8},
  page:{maxWidth:1120,margin:"0 auto",padding:"40px 28px"},
  card:{background:C.white,borderRadius:18,padding:24,border:`1px solid ${C.line}`,boxShadow:SHADOW},
  h1:{fontFamily:font,fontSize:34,color:C.dark,fontWeight:800,letterSpacing:"-0.02em",marginBottom:6},
  h2:{fontFamily:font,fontSize:22,color:C.dark,fontWeight:700,letterSpacing:"-0.01em",marginBottom:14},
  h3:{fontFamily:font,fontSize:16,color:C.dark,fontWeight:700,marginBottom:10},
  sub:{fontSize:14,color:C.gray,marginBottom:24,lineHeight:1.6},
  label:{fontSize:11,letterSpacing:"0.1em",color:C.gray,textTransform:"uppercase",fontWeight:700,display:"block",marginBottom:6},
  input:{width:"100%",padding:"11px 14px",borderRadius:12,border:`1.5px solid ${C.line}`,background:C.white,fontSize:14,fontFamily:fb,color:C.dark,outline:"none",boxSizing:"border-box",transition:"border-color .2s,box-shadow .2s"},
  select:{width:"100%",padding:"11px 14px",borderRadius:12,border:`1.5px solid ${C.line}`,background:C.white,fontSize:14,fontFamily:fb,color:C.dark,outline:"none",boxSizing:"border-box"},
  btn:{padding:"12px 24px",borderRadius:100,border:"none",background:C.rose,color:C.white,cursor:"pointer",fontSize:13.5,fontFamily:fb,fontWeight:700,letterSpacing:"0.01em",transition:"transform .2s,box-shadow .2s,opacity .2s"},
  btnO:{padding:"11px 22px",borderRadius:100,border:`1.5px solid ${C.line}`,background:"transparent",color:C.dark,cursor:"pointer",fontSize:13.5,fontFamily:fb,fontWeight:600,transition:"border-color .2s,color .2s"},
  btnR:{padding:"12px 24px",borderRadius:100,border:"none",background:C.rose,color:C.white,cursor:"pointer",fontSize:13.5,fontFamily:fb,fontWeight:700},
  ...
  chip:(on)=>({padding:"9px 17px",borderRadius:100,cursor:"pointer",fontSize:13,fontFamily:fb,border:`1.5px solid ${on?C.rose:C.line}`,background:on?C.roseLight:C.white,color:on?C.rose:C.gray,fontWeight:on?700:400,transition:"all .18s",display:"inline-flex",alignItems:"center",gap:6}),
  chipR:(on)=>({padding:"9px 17px",borderRadius:100,cursor:"pointer",fontSize:13,fontFamily:fb,border:`1.5px solid ${on?C.rose:C.line}`,background:on?C.roseLight:C.white,color:on?C.rose:C.gray,fontWeight:on?700:400,transition:"all .18s"}),
  badge:(c)=>({display:"inline-block",padding:"3px 10px",borderRadius:7,fontSize:11,fontWeight:700,background:c==="g"?C.tealBg:c==="r"?C.roseLight:c==="gold"?C.goldBg:C.muted,color:c==="g"?C.teal:c==="r"?C.rose:c==="gold"?C.gold:C.gray}),
  bar:{height:5,borderRadius:3,background:C.line,overflow:"hidden"},
  fill:(p,over)=>({height:"100%",width:`${Math.min(Math.max(p,0),100)}%`,borderRadius:3,background:over?C.rose:`linear-gradient(90deg,${C.violet},${C.rose})`,transition:"width 1s cubic-bezier(.2,.7,.2,1)"}),
  tab:(on)=>({padding:"8px 16px",borderRadius:10,border:"none",cursor:"pointer",background:on?C.roseLight:"transparent",color:on?C.rose:C.gray,fontSize:13.5,fontFamily:fb,fontWeight:on?700:500,transition:"all .18s"}),
};
...
function calcScenarios(survey){
  ...
  const meta={
    basic:{label:"Базовый",icon:"🌿",color:C.teal,bullets:["Банкетный зал или шатёр","Стандартное меню и напитки","1 фотограф","Базовый декор","DJ и звук"]},
    comfort:{label:"Комфорт",icon:"✨",color:C.gold,bullets:["Площадка с атмосферой","Авторское меню + алкоголь","Фотограф + видеограф","Полный декор и флористика","Ведущий + DJ"]},
    premium:{label:"Премиум",icon:"🏆",color:C.rose,bullets:["Топовая площадка","Premium-меню и open bar","2 фотографа + видео + аэро","Дизайнерский декор","Шоу-программа, живая музыка"]},
  };
...
function AuthPage({onLogin,prefill,onBack,agency}){
  ...
  return(
    <div style={{minHeight:"100vh",background:`linear-gradient(135deg,${C.roseLight} 0%,${C.bg} 50%,${C.violetLight} 100%)`,display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
      <link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;600;700;800&display=swap" rel="stylesheet"/>
      <div style={{...S.card,maxWidth:400,width:"100%"}}>
        <div style={{textAlign:"center",marginBottom:24}}>
          <h1 style={{fontFamily:font,fontSize:34,color:C.dark,margin:0,fontWeight:800,letterSpacing:"-0.02em"}}>Tot<span style={{color:C.rose}}>Day</span>{agency&&<span style={{fontFamily:fb,fontSize:11,fontWeight:700,color:C.rose,background:C.roseLight,padding:"2px 8px",borderRadius:6,marginLeft:8,verticalAlign:"middle"}}>AGENCY</span>}</h1>
        ...
        {prefill&&!agency&&(
          <div style={{background:C.roseLight,borderRadius:12,padding:"10px 14px",marginBottom:18,fontSize:13,color:C.rose,textAlign:"center",border:`1px solid ${C.roseMid}`}}>
            ✓ {prefill.city} · {prefill.guests} гостей · смета готова
          </div>
        )}
        <div style={{display:"flex",gap:6,marginBottom:20,background:C.muted,borderRadius:12,padding:4}}>
...
function LandingPage({onStart,onLogin,onAgency}){
  const FEAT_COLORS=[C.rose,C.violet,C.teal,C.gold,C.violet,C.rose];
  const features=[
    ...
    <div style={{background:C.bg,minHeight:"100vh",color:C.dark,fontFamily:fb}}>
      ...
      <style>{`
        *{-webkit-font-smoothing:antialiased}
        ...
        ::selection{background:${C.roseLight}}
      `}</style>
      <nav style={{...S.nav,maxWidth:"none"}}>
        <div style={{maxWidth:1120,margin:"0 auto",width:"100%",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div style={S.logo}>Tot<span style={{color:C.rose}}>Day</span></div>
          <div style={{display:"flex",gap:10,alignItems:"center"}}>
            <button style={{background:"none",border:"none",color:C.gray,fontSize:13.5,fontWeight:500,cursor:"pointer",fontFamily:fb}} onClick={onAgency}>Для агентств</button>
      ...
      <section style={{maxWidth:1120,margin:"0 auto",padding:"88px 28px 56px",textAlign:"center",position:"relative"}}>
        <div style={{position:"absolute",top:"-5%",left:"50%",transform:"translateX(-50%)",width:800,height:480,maxWidth:"120%",background:`radial-gradient(ellipse at 40% 50%,${C.roseLight} 0%,${C.violetLight} 50%,transparent 75%)`,filter:"blur(8px)",zIndex:0,pointerEvents:"none"}}/>
        <div style={{position:"relative",zIndex:1}}>
          <div className="lf" style={{display:"inline-flex",alignItems:"center",gap:8,fontSize:12.5,fontWeight:700,letterSpacing:"0.14em",textTransform:"uppercase",color:C.rose,marginBottom:24,background:C.roseLight,padding:"6px 16px",borderRadius:100}}>
            Планирование свадьбы
          </div>
          <h1 className="lf2" style={{fontFamily:font,fontWeight:800,fontSize:"clamp(40px,7vw,72px)",lineHeight:1.02,letterSpacing:"-0.03em",margin:"0 0 22px"}}>
            Ваша свадьба —<br/><span style={{background:`linear-gradient(90deg,${C.violet},${C.rose})`,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>понятная и управляемая</span>
          </h1>
          <p className="lf2" style={{maxWidth:560,margin:"0 auto 36px",fontSize:17,color:C.ink2,lineHeight:1.6}}>
            Смета на реальных ценах, гости, подрядчики, тайминг и сайт-приглашение. Всё в одном спокойном месте — без бесконечных таблиц и хаоса.
          </p>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:20}}>
          {steps.map((s,i)=>(
            <div key={s.n} style={{padding:"4px 8px"}}>
              <div style={{fontFamily:font,fontSize:34,fontWeight:800,background:`linear-gradient(90deg,${C.violet},${C.rose})`,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",lineHeight:1.1}}>{s.n}</div>
              <div style={{fontFamily:font,fontSize:20,fontWeight:700,margin:"6px 0 6px"}}>{s.t}</div>
      ...
      <section style={{maxWidth:1120,margin:"0 auto",padding:"56px 28px"}}>
        <div style={{textAlign:"center",marginBottom:36}}>
          <h2 style={{fontFamily:font,fontSize:"clamp(26px,4vw,36px)",fontWeight:800,letterSpacing:"-0.02em"}}>Всё, что нужно для свадьбы</h2>
          <p style={{color:C.gray,fontSize:15,marginTop:8}}>Инструменты уровня свадебного агентства — в ваших руках</p>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))",gap:18}}>
          {features.map((f,i)=>(
            <div key={f.t} className="lcard" style={{...S.card,borderTop:`3px solid ${FEAT_COLORS[i]}`}}>
              <div style={{fontFamily:font,fontSize:19,fontWeight:700,marginBottom:8,color:C.dark}}>{f.t}</div>
      ...
      <section style={{maxWidth:1120,margin:"0 auto",padding:"40px 28px 88px"}}>
        <div style={{background:`linear-gradient(135deg,${C.violet},${C.rose})`,borderRadius:24,padding:"56px 32px",textAlign:"center",boxShadow:SHADOW_HOVER}}>
          <h2 style={{fontFamily:font,fontSize:"clamp(26px,4vw,38px)",fontWeight:800,letterSpacing:"-0.02em",marginBottom:14,color:C.white}}>Начнём планировать?</h2>
          <p style={{color:"rgba(255,255,255,.8)",fontSize:16,marginBottom:28,maxWidth:440,margin:"0 auto 28px"}}>Пара вопросов — и вы увидите смету своей свадьбы. Бесплатно.</p>
          <button style={{...S.btn,padding:"15px 36px",fontSize:15,background:C.white,color:C.rose}} onClick={onStart}>Начать →</button>
        </div>
      </section>

      <footer style={{borderTop:`1px solid ${C.line}`,padding:"28px",textAlign:"center",color:C.gray,fontSize:13}}>
        <div style={{...S.logo,justifyContent:"center",marginBottom:8}}>Tot<span style={{color:C.rose}}>Day</span></div>
  ...
  return(
    <div style={{minHeight:"100vh",background:`linear-gradient(135deg,${C.roseLight} 0%,${C.bg} 50%,${C.violetLight} 100%)`,display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
      ...
      <div style={{maxWidth:520,width:"100%"}}>
        <div style={{textAlign:"center",marginBottom:8}}><div style={S.logo}>Tot<span style={{color:C.rose}}>Day</span></div></div>
        <div style={{textAlign:"center",fontSize:12,color:C.rose,letterSpacing:"0.14em",textTransform:"uppercase",fontWeight:700,marginBottom:18}}>Шаг {step+1} из {steps.length}</div>
        <div style={{height:5,background:C.line,borderRadius:3,marginBottom:28,overflow:"hidden"}}>
          <div style={{height:"100%",width:`${(step+1)/steps.length*100}%`,background:`linear-gradient(90deg,${C.violet},${C.rose})`,borderRadius:3,transition:"width .4s cubic-bezier(.2,.7,.2,1)"}}/>
        </div>
        <div style={{...S.card,padding:"36px 28px"}}>
          <h2 style={{fontFamily:font,fontSize:26,fontWeight:800,letterSpacing:"-0.02em",textAlign:"center",marginBottom:28}}>{cur.q}</h2>
          {cur.body}
        </div>
...
function Survey1Page({onComplete,initial}){
  ...
  const STEPS=[
    ...
    {title:"Где будет свадьба?",sub:"Город влияет на ценовой расчёт",ok:d.city,body:(
    ...
    {title:"Формат свадьбы",sub:"Влияет на подбор площадок",ok:d.format,body:(
      <div style={{display:"flex",flexDirection:"column",gap:10}}>
        {FORMATS.map(f=>(
          <div key={f.id} onClick={()=>set("format",f.id)} style={{...S.card,padding:"14px 18px",cursor:"pointer",display:"flex",alignItems:"center",gap:14,border:`2px solid ${d.format===f.id?C.rose:C.line}`,background:d.format===f.id?C.roseLight:C.white}}>
            <span style={{fontSize:22}}>{f.icon}</span>
            <div style={{flex:1}}><div style={{fontWeight:700,fontSize:14}}>{f.label}</div><div style={{fontSize:12,color:C.gray}}>{f.sub}</div></div>
            {d.format===f.id&&<span style={{color:C.rose}}>✓</span>}
          </div>
        ))}
      </div>
    )},
    {title:"Бюджет на свадьбу",sub:"Используем для расчёта. «Пока не знаю» — рассчитаем сами",ok:d.budgetLabel,body:(
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        {BUDGET_OPTIONS.map(b=>(
          <div key={b.label} onClick={()=>{set("budgetLabel",b.label);set("budget",b.val);}} style={{...S.card,padding:"13px 18px",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",border:`2px solid ${d.budgetLabel===b.label?C.rose:C.line}`,background:d.budgetLabel===b.label?C.roseLight:C.white}}>
            <span style={{fontWeight:d.budgetLabel===b.label?700:400,fontSize:14}}>{b.label}</span>
            {d.budgetLabel===b.label&&<span style={{color:C.rose}}>✓</span>}
          </div>
        ))}
      </div>
    )},
  ...
  return(
    <div style={{...S.page,maxWidth:600}}>
      <div style={{marginBottom:24,textAlign:"center"}}>
        <span style={{...S.badge("r"),fontSize:12,padding:"4px 14px",marginBottom:12,display:"inline-block"}}>Быстрый расчёт · шаг {step+1} из {STEPS.length}</span>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))",gap:20,marginBottom:32}}>
        {scenarios.map(sc=>(
          <div key={sc.tier} onClick={()=>setChosen(sc.tier)}
            style={{...S.card,cursor:"pointer",border:`2px solid ${chosen===sc.tier?sc.color:C.line}`,background:chosen===sc.tier?`${sc.color}10`:C.white,transition:"all 0.15s",position:"relative",borderTop:`4px solid ${sc.color}`}}>
            ...
            <div style={{fontFamily:font,fontSize:26,fontWeight:800,color:C.dark,marginBottom:2}}>{fmt(sc.total)} ₽</div>
...
function Survey2Page({onComplete,initial}){
  ...
  const Chips=({field,max,opts})=>(
    <div style={{display:"flex",flexWrap:"wrap",gap:8,marginTop:8}}>
      {opts.map(([v,l])=>{
        const on=d[field].includes(v);
        const dis=!on&&max&&d[field].length>=max;
        return <span key={v} style={{...S.chip(on),opacity:dis?0.4:1,cursor:dis?"not-allowed":"pointer"}} onClick={()=>!dis&&toggle(field,v,max)}>{l}</span>;
      })}
    </div>
  );
  const Cards=({field,opts})=>(
    <div style={{display:"flex",flexDirection:"column",gap:8,marginTop:8}}>
      {opts.map(([v,l,sub])=>(
        <div key={v} onClick={()=>set(field,v)} style={{...S.card,padding:"13px 16px",cursor:"pointer",display:"flex",alignItems:"center",gap:12,border:`2px solid ${d[field]===v?C.rose:C.line}`,background:d[field]===v?C.roseLight:C.white}}>
          ...
          {d[field]===v&&<span style={{color:C.rose}}>✓</span>}
        </div>
      ))}
    </div>
  );
  const STEPS=[
    ...
    {title:"Что точно НЕ хотите?",sub:"Передадим подрядчикам в ТЗ",body:(
      <div style={{display:"flex",flexWrap:"wrap",gap:8,marginTop:8}}>
        {[["contests","Пошлые конкурсы"],["toasts","Длинные тосты"],["ransom","Выкуп"],["oldhost","Тамада старого формата"],["official","Много официоза"],["boring","Банкет как у всех"],["karaoke","Караоке"],["kids","Детские активности"],["envelopes","Сбор денег в конвертах"]].map(([v,l])=>{
          const on=d.dontWant.includes(v);
          return <span key={v} style={S.chipR(on)} onClick={()=>toggle("dontWant",v)}>{l}</span>;
        })}
      </div>
    )},
    ...
    {title:"Важность комфорта гостей",sub:"Трансфер, отель, детская зона...",body:(
      <div style={{marginTop:16}}>
        <div style={{display:"flex",justifyContent:"space-between",gap:8}}>
          {[1,2,3,4,5].map(n=>(
            <div key={n} onClick={()=>set("guestComfort",n)} style={{flex:1,height:52,borderRadius:10,display:"flex",alignItems:"center",justifyContent:"center",border:`2px solid ${d.guestComfort===n?C.rose:C.line}`,background:d.guestComfort===n?C.rose:C.white,color:d.guestComfort===n?C.white:C.gray,fontWeight:700,fontSize:18,cursor:"pointer"}}>
              {n}
            </div>
          ))}
        </div>
    ...
    {title:"Свадьбы, которые вам нравятся",sub:"Лайкайте — самый важный вопрос для стиля",body:(
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginTop:8}}>
        {WEDDING_PHOTOS.map(p=>{
          ...
          return(
            <div key={p.id} onClick={()=>toggle("likedPhotos",p.id)} style={{borderRadius:10,overflow:"hidden",cursor:"pointer",border:`3px solid ${on?C.rose:"transparent"}`,transition:"all 0.13s"}}>
              <div style={{background:on?C.roseLight:C.muted,height:80,display:"flex",alignItems:"center",justifyContent:"center",fontSize:32}}>{p.emoji}</div>
              <div style={{padding:"7px 10px",background:on?C.roseLight:C.white,fontSize:11,fontWeight:on?700:400,color:on?C.rose:C.dark}}>{p.label}</div>
            </div>
          );
        })}
      </div>
...
function BudgetPage({survey,cats,setCats,onGoToVendors}){
  ...
  const STAT_COLORS=[C.violet,totalPlan>totalBudget?C.rose:C.teal,(totalBudget-totalPlan)>=0?C.teal:C.rose,C.rose,leftBudget>=0?C.teal:C.rose];
  ...
  return(
    <div className="td-page" style={S.page}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(155px,1fr))",gap:14,marginBottom:18}}>
        <div style={{...S.card,textAlign:"center",borderTop:`3px solid ${C.violet}`}}>
          <div style={{fontSize:10,color:C.gray,letterSpacing:"0.08em",textTransform:"uppercase",marginBottom:4}}>Общий бюджет</div>
          {editingBudget?(
            <input type="number" min="0" autoFocus style={{...S.input,textAlign:"center",fontWeight:700,fontSize:16,color:C.dark,padding:"4px 8px"}}
            ...
            <div style={{fontFamily:font,fontSize:18,color:C.dark,fontWeight:800,cursor:"pointer",borderBottom:`1px dashed ${C.rose}`}} onClick={()=>{setBudgetInput(String(totalBudget));setEditingBudget(true);}}>
              {fmt(totalBudget)} ₽
            </div>
          )}
        ...
        {[
          {l:"Запланировано",v:`${fmt(totalPlan)} ₽`,c:totalPlan>totalBudget?C.rose:C.dark,ct:totalPlan>totalBudget?C.rose:C.teal,warn:totalPlan>totalBudget?`превышает на ${fmt(totalPlan-totalBudget)} ₽`:""},
          {l:"Свободно в плане",v:`${fmt(totalBudget-totalPlan)} ₽`,c:(totalBudget-totalPlan)>=0?C.teal:C.rose,ct:(totalBudget-totalPlan)>=0?C.teal:C.rose},
          {l:"Потрачено",v:`${fmt(totalActual)} ₽`,c:C.rose,ct:C.rose},
          {l:"Остаток денег",v:`${fmt(leftBudget)} ₽`,c:leftBudget>=0?C.teal:C.rose,ct:leftBudget>=0?C.teal:C.rose},
        ].map(s=>(
          <div key={s.l} style={{...S.card,textAlign:"center",borderTop:`3px solid ${s.ct}`}}>
            <div style={{fontSize:10,color:C.gray,letterSpacing:"0.08em",textTransform:"uppercase",marginBottom:4}}>{s.l}</div>
            <div style={{fontFamily:font,fontSize:18,color:s.c,fontWeight:800}}>{s.v}</div>
      ...
      <div style={{...S.card,padding:"14px 20px",marginBottom:18}}>
        <div style={{display:"flex",justifyContent:"space-between",fontSize:12,color:C.gray,marginBottom:6}}>
          ...
          <span style={{color:C.rose,fontWeight:700}}>{Math.round(budgetPct)}%</span>
        </div>
      ...
      {activeTab==="plan"&&(
        <div style={S.card}>
          ...
          {showAddCat&&(
            <div style={{background:C.violetLight,borderRadius:10,padding:16,marginBottom:16,display:"flex",gap:10,flexWrap:"wrap",alignItems:"flex-end"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"28px 1fr 100px 100px 100px 100px 70px",gap:8,padding:"6px 0 10px",borderBottom:`2px solid ${C.line}`,fontSize:10,color:C.gray,letterSpacing:"0.07em",textTransform:"uppercase",fontWeight:700}}>
            <div/><div>Категория</div>
          ...
          {cats.map((c,i)=>{
            ...
            return(
              <div key={c.id} style={{borderBottom:i<cats.length-1?`1px solid ${C.muted}`:"none"}}>
                <div style={{display:"grid",gridTemplateColumns:"28px 1fr 100px 100px 100px 100px 70px",gap:8,padding:"12px 0",alignItems:"center",cursor:"pointer"}} onClick={()=>toggleExpand(c.id)}>
                  <span style={{fontSize:16,width:28,height:28,borderRadius:8,background:C.muted,display:"flex",alignItems:"center",justifyContent:"center"}}>{c.icon}</span>
                  <div>
                    <div style={{fontSize:13.5,fontWeight:600,display:"flex",alignItems:"center",gap:6}}>
                      {c.name}
                      {over&&<span style={S.badge("r")}>перерасход</span>}
                    </div>
                    <div style={{fontSize:10,color:C.rose,marginTop:2}}>{c.rec}</div>
                  ...
                  <div style={{textAlign:"right"}} onClick={e=>e.stopPropagation()}>
                    {editPlanId===c.id?(
                      <input type="number" min="0" autoFocus style={{...S.input,width:"100%",padding:"3px 7px",fontSize:13,textAlign:"right"}}
                        ...
                        onFocus={e=>e.target.select()} onBlur={()=>setEditPlanId(null)} onKeyDown={e=>e.key==="Enter"&&setEditPlanId(null)}/>
                    ):(
                      <span style={{fontSize:13,color:C.dark,fontWeight:600,cursor:"text",borderBottom:`1px dashed ${C.rose}`}} onClick={()=>setEditPlanId(c.id)}>
                        {fmt(c.plan)} ₽
                      </span>
                    )}
                  </div>
                  <div style={{textAlign:"right"}} onClick={e=>e.stopPropagation()}>
                    {editAvansId===c.id?(
                      <input type="number" min="0" autoFocus style={{...S.input,width:"100%",padding:"3px 7px",fontSize:13,textAlign:"right"}}
                        ...
                        onFocus={e=>e.target.select()} onBlur={()=>setEditAvansId(null)} onKeyDown={e=>e.key==="Enter"&&setEditAvansId(null)}/>
                    ):(
                  ...
                  <div style={{textAlign:"right",fontSize:13,fontWeight:700,color:C.ink2}}>{fmt(Math.max(0,c.plan-(c.avans||0)))} ₽</div>
                  <div style={{textAlign:"right",fontSize:13,color:over?C.rose:C.teal,fontWeight:600}}>{fmt(c.actual)} ₽</div>
                  <div style={{display:"flex",gap:4,justifyContent:"flex-end"}} onClick={e=>e.stopPropagation()}>
                    <button style={{...S.btnSm,color:C.violet,borderColor:C.line,padding:"3px 8px"}} onClick={()=>onGoToVendors(c.id)}>→</button>
                    <button style={{...S.btnSm,color:C.rose,borderColor:C.line,padding:"3px 8px"}} onClick={()=>removeCat(c.id)}>✕</button>
                  </div>
                </div>
                {c.expanded&&(
                  <div style={{background:C.muted,borderRadius:10,padding:"10px 14px",marginBottom:10,marginLeft:36}}>
                    ...
                    {c.items.length>0?c.items.map(item=>(
                      <div key={item} style={{display:"flex",alignItems:"center",gap:10,padding:"5px 0",borderBottom:`1px solid ${C.line}`}}>
          ...
          {(()=>{
            ...
            const service=Math.round(totalPlan*0.10);
            const deposit=50000;
            return(
              <div style={{marginTop:18,borderTop:`2px solid ${C.line}`,paddingTop:16}}>
                ...
                <div style={{display:"flex",justifyContent:"space-between",padding:"7px 0",fontSize:13,borderBottom:`1px solid ${C.line}`}}>
                  ...
                  <strong style={{color:C.rose}}>{fmt(toPay)} ₽</strong>
                </div>
                ...
                <p style={{fontSize:11,color:C.gray,marginTop:10,lineHeight:1.5}}>Сервисный сбор и депозит — справочно, не входят в сумму сметы.</p>
              </div>
            );
          })()}
        </div>
          ...
          <div style={S.card}>
            ...
            {txHistory.length===0
              ...
              :txHistory.map((tx,i)=>{
                ...
                return(
                  <div key={tx.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"11px 0",borderBottom:i<txHistory.length-1?`1px solid ${C.muted}`:"none"}}>
...
function VendorsPage({survey,initCat}){
  ...
  return(
    <div className="td-page" style={S.page}>
          ...
          return(
            <div key={v.id} style={{...S.card,display:"flex",flexDirection:"column",position:"relative"}}>
              ...
              <div style={{display:"flex",gap:10,marginBottom:10}}>
                <div style={{width:42,height:42,borderRadius:10,background:C.violetLight,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>{cat?.icon}</div>
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",paddingTop:10,borderTop:`1px solid ${C.muted}`}}>
                <div><div style={{fontWeight:700,color:C.rose,fontSize:14}}>от {fmt(v.priceFrom)} ₽</div><div style={{fontSize:10,color:C.gray}}>⭐ {v.rating} · {v.reviews} отзывов</div></div>
      ...
      {modal&&(
        <div style={{position:"fixed",inset:0,background:"rgba(24,24,27,0.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:500,padding:20}}>
          <div style={{...S.card,maxWidth:460,width:"100%"}}>
            ...
            <div style={{background:C.muted,borderRadius:10,padding:14,fontSize:13,lineHeight:1.9,marginBottom:18}}>
...
function GuestsPage({slug,guests,setGuests}){
  ...
  const STAT_META=[
    {l:"Всего",v:stats.total,c:C.dark,bc:C.violet},
    {l:"Подтверждено",v:stats.yes,c:C.teal,bc:C.teal},
    {l:"Ожидает",v:stats.wait,c:C.gold,bc:C.gold},
    {l:"Трансфер",v:stats.transfer,c:C.violet,bc:C.violet},
    {l:"Проживание",v:stats.lodging,c:C.rose,bc:C.rose},
  ];
  return(
    <div className="td-page" style={S.page}>
      <h2 style={S.h1}>Список гостей</h2>
      <p style={S.sub}>Ссылка на сайт: <strong style={{color:C.rose}}>totday.app/{slug}</strong></p>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))",gap:14,marginBottom:22}}>
        {STAT_META.map(s=>(
          <div key={s.l} className="td-lift" style={{...S.card,textAlign:"center",borderTop:`3px solid ${s.bc}`}}>
            <div style={{fontSize:10,color:C.gray,letterSpacing:"0.08em",textTransform:"uppercase",marginBottom:6}}>{s.l}</div>
            <div style={{fontFamily:font,fontSize:28,color:s.c,fontWeight:800}}>{s.v}</div>
          </div>
        ))}
      </div>
      <div style={{...S.card,marginBottom:20}}>
        ...
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
            <thead><tr style={{borderBottom:`2px solid ${C.line}`}}>
              {["Имя","Кем приходится","Сторона","RSVP","Стол","Трансфер","Прожив.",""].map(h=><th key={h} style={{padding:"9px 10px",textAlign:"left",fontSize:10,color:C.gray,letterSpacing:"0.08em",textTransform:"uppercase",fontWeight:700}}>{h}</th>)}
            </tr></thead>
            <tbody>{filtered.map(g=>(
              <Fragment key={g.id}>
              <tr style={{borderBottom:expanded===g.id?"none":`1px solid ${C.line}`}}>
                <td style={{padding:"11px 10px",fontWeight:600}}>{g.name}</td>
                ...
                <tr style={{borderBottom:`1px solid ${C.line}`,background:C.bg}}>
                  <td colSpan={8} style={{padding:"14px 10px"}}>
                    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:12}}>
                      ...
                      <div style={{display:"flex",alignItems:"flex-end",paddingBottom:6}}><button style={{...S.btnSm,color:C.rose,borderColor:C.roseLight}} onClick={()=>del(g.id)}>Удалить гостя</button></div>
                    </div>
                  </td>
                </tr>
              )}
...
function TimingPage({survey,timing,setTiming}){
  ...
  const BLOCK_COLORS={before:C.violet,day:C.rose};
  const add=()=>{
  ...
  return(
    <div className="td-page" style={S.page}>
      ...
      {blocks.map(b=>{
        ...
        return(
          <div key={b.id} style={{...S.card,marginBottom:18,borderTop:`3px solid ${BLOCK_COLORS[b.id]}`}}>
            <div style={{fontSize:11,letterSpacing:"0.14em",textTransform:"uppercase",color:BLOCK_COLORS[b.id],fontWeight:700,marginBottom:14}}>{b.label}</div>
            {items.length===0?(
              ...
              <div>
                {items.map((t,i)=>(
                  <div key={t.id} style={{display:"grid",gridTemplateColumns:"78px 1fr 36px",gap:14,alignItems:"center",padding:"13px 0",borderTop:i>0?`1px solid ${C.line}`:"none"}}>
                    <input style={{...S.input,padding:"6px 10px",fontFamily:font,fontSize:16,textAlign:"center",fontWeight:700}} value={t.time||""} onChange={e=>upd(t.id,{time:e.target.value})} placeholder="—:—"/>
...
// ─── INVITE PREVIEW (Bug #6: вынесен из InvitePage) ─────────────────────────
  ...
  return(
    <div style={{background:t.bg,color:t.textColor,borderRadius:14,padding:"36px 28px",fontFamily:font,border:`1px solid ${t.accent}30`}}>
      <div style={{textAlign:"center",marginBottom:24}}>
        ...
        <h1 style={{fontSize:26,margin:"0 0 10px",lineHeight:1.2,fontWeight:800}}>{c.title}</h1>
      ...
      <div style={{borderTop:`1px solid ${t.accent}40`,borderBottom:`1px solid ${t.accent}40`,padding:"14px 0",margin:"0 0 18px",textAlign:"center"}}>
        ...
        <div style={{fontSize:14,fontWeight:700}}>{c.venue}</div>
      ...
      <div style={{textAlign:"center"}}>
        <button style={{padding:"10px 26px",borderRadius:26,border:`2px solid ${t.accent}`,background:t.accent,color:t.id==="luxury"?C.dark:C.white,cursor:"pointer",fontSize:12,fontFamily:font,fontWeight:700}}>Подтвердить участие</button>
      </div>
    </div>
  );
}
        ...
        <div style={{display:"flex",gap:12,flexWrap:"wrap"}}>
          {INVITE_TEMPLATES.map(t2=>(
            <div key={t2.id} onClick={()=>setTmpl(t2.id)} style={{cursor:"pointer",textAlign:"center"}}>
              <div style={{width:70,height:44,borderRadius:8,background:t2.bg,border:`3px solid ${tmplId===t2.id?C.rose:C.line}`,display:"flex",alignItems:"center",justifyContent:"center",transition:"border-color .18s"}}>
                <span style={{color:t2.accent,fontFamily:font,fontSize:13,fontWeight:700}}>Aa</span>
              </div>
              <div style={{fontSize:11,color:tmplId===t2.id?C.rose:C.gray,marginTop:3,fontWeight:tmplId===t2.id?700:400}}>{t2.name}</div>
            </div>
          ))}
        </div>
      </div>
        ...
        <div>
          <div style={{...S.card,marginBottom:14}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <span style={{fontSize:13,color:C.rose,fontWeight:700}}>totday.app/{slug}</span>
...
function CountdownTimer({date,names}){
  ...
  if(!t)return(
    <div style={{...S.card,textAlign:"center",background:`linear-gradient(135deg,${C.roseLight},${C.violetLight})`,padding:"28px 24px",border:"none"}}>
      <div style={{fontSize:13.5,color:C.gray}}>Укажите дату свадьбы в «Быстром расчёте», чтобы запустить обратный отсчёт</div>
    </div>
  );
  if(t.passed)return(
    <div style={{...S.card,textAlign:"center",background:`linear-gradient(135deg,${C.violet},${C.rose})`,padding:"32px",border:"none"}}>
      <div style={{fontFamily:font,fontSize:24,color:C.white,fontWeight:800}}>🎉 Поздравляем — ваш день настал!</div>
    </div>
  );
  const units=[["дней",t.days],["часов",t.hours],["минут",t.mins],["секунд",t.secs]];
  return(
    <div style={{background:`linear-gradient(135deg,${C.roseLight} 0%,${C.violetLight} 100%)`,border:`1px solid ${C.line}`,borderRadius:24,padding:"30px 36px",boxShadow:SHADOW,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:20}}>
      <div style={{textAlign:"left"}}>
        <div style={{fontSize:11,letterSpacing:"0.16em",textTransform:"uppercase",color:C.gray,marginBottom:6}}>До свадьбы</div>
        {names&&<div style={{fontFamily:font,fontSize:22,fontWeight:800,color:C.dark}}>{names}</div>}
      </div>
      <div style={{display:"flex",gap:18,textAlign:"center"}}>
        {units.map(([label,val],i)=>(
          <div key={label} style={{display:"flex",alignItems:"flex-start",gap:18}}>
            <div style={{minWidth:50}}>
              <div style={{fontFamily:font,fontSize:40,fontWeight:800,lineHeight:1,letterSpacing:"-0.02em",background:`linear-gradient(135deg,${C.violet},${C.rose})`,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",fontVariantNumeric:"tabular-nums"}}>{String(val).padStart(2,"0")}</div>
              <div style={{fontSize:10,letterSpacing:"0.14em",textTransform:"uppercase",color:C.gray,marginTop:8}}>{label}</div>
            </div>
            {i<units.length-1&&<div style={{fontFamily:font,fontSize:32,color:C.roseMid,lineHeight:1}}>:</div>}
          </div>
        ))}
      </div>
    </div>
...
function Dashboard({user,survey,cats,guests,onNav}){
  ...
  const NAV_COLORS=[C.violet,C.rose,C.teal,C.gold,C.violet,C.rose];
  return(
    <div className="td-page" style={S.page}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(155px,1fr))",gap:14,marginBottom:22}}>
        {[
          {l:"Бюджет",v:`${fmt(total)} ₽`,c:C.violet,bc:C.violet},
          {l:"Потрачено",v:`${fmt(totalActual)} ₽`,c:C.rose,bc:C.rose},
          {l:"Остаток",v:`${fmt(total-totalActual)} ₽`,c:C.teal,bc:C.teal},
          {l:"Гостей",v:guests.length||survey?.guests||"—",c:C.gold,bc:C.gold},
        ].map(s=>(
          <div key={s.l} style={{...S.card,textAlign:"center",borderTop:`3px solid ${s.bc}`}}>
            <div style={{fontSize:10,color:C.gray,letterSpacing:"0.08em",textTransform:"uppercase",marginBottom:3}}>{s.l}</div>
            <div style={{fontFamily:font,fontSize:17,color:s.c,fontWeight:800}}>{s.v}</div>
          </div>
        ))}
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:18,marginBottom:20}}>
        <div style={S.card}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",fontSize:12,color:C.gray,marginBottom:5}}>
            <span>Потрачено {fmt(totalActual)} ₽</span>
            <span>{total>0?Math.round(totalActual/total*100):0}%</span>
          </div>
        ...
        <div style={S.card}>
          ...
          {tasks.map((t,i)=>(
            <div key={i} style={{display:"flex",alignItems:"center",gap:8,marginBottom:7}}>
              <div style={{width:16,height:16,borderRadius:4,border:`2px solid ${t.done?C.rose:C.line}`,background:t.done?C.rose:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(185px,1fr))",gap:14}}>
        {[
          ...
          <div key={n.tab} className="td-lift" style={{...S.card,cursor:"pointer",borderTop:`3px solid ${NAV_COLORS[i]}`}} onClick={()=>onNav(n.tab)}>
  ...
  return(
    <div className="td-page" style={S.page}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:16,marginBottom:24}}>
        {[
          {l:"Активных свадеб",v:active,c:C.rose,bc:C.rose},
          {l:"Всего проектов",v:weddings.length,c:C.dark,bc:C.violet},
          {l:"Суммарный бюджет",v:`${(totalBudget/1000000).toFixed(1)} млн ₽`,c:C.gold,bc:C.gold},
          {l:"Получено оплат",v:`${(totalPaid/1000000).toFixed(1)} млн ₽`,c:C.teal,bc:C.teal},
        ].map(s=>(
          <div key={s.l} className="td-lift" style={{...S.card,textAlign:"center",borderTop:`3px solid ${s.bc}`}}>
            <div style={{fontSize:10,color:C.gray,letterSpacing:"0.08em",textTransform:"uppercase",marginBottom:8}}>{s.l}</div>
            <div style={{fontFamily:font,fontSize:26,color:s.c,fontWeight:800}}>{s.v}</div>
          </div>
        ))}
      </div>
          ...
          return(
            <div key={w.id} className="td-lift" style={{...S.card,cursor:"pointer",position:"relative"}} onClick={()=>onOpen&&onOpen(w)}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
                <div style={{fontFamily:font,fontSize:20,fontWeight:800}}>{w.couple}</div>
              ...
              <div style={{fontSize:11,color:C.gray,marginBottom:5,display:"flex",justifyContent:"space-between"}}>
                <span>Оплачено</span><span style={{color:C.teal,fontWeight:700}}>{paidPct}%</span>
              </div>
...
export default function App(){
  ...
  // Bug #2: setCats вызывается вне setSurvey — нет двойного вызова в Strict Mode
  ...
  // Bug #4: при открытии свадьбы агентства — загружаем её данные
  const handleOpenWedding=useCallback((w)=>{
    setOpenWedding(w);
    setCats(w._cats||makeCatsFromBudget(w.budget||1500000,{guests:w.guests,city:w.city,format:"restaurant"},"comfort"));
    setGuests(w._guests||[]);
    setTiming(w._timing||[]);
    setTab("budget");
  },[]);

  // Bug #4: при возврате в кабинет агентства — сохраняем данные в wedding объект
  const handleGoToAgency=useCallback(()=>{
    if(openWedding){
  ...
  // Bug #3: logout очищает ВСЕ ключи включая td_role и td_agency_weddings; сбрасывает role
  const logout=()=>{
    ...
    <div style={S.app}>
      ...
      <style>{`
        *{-webkit-font-smoothing:antialiased;text-rendering:optimizeLegibility}
        button{transition:transform .2s,box-shadow .2s,border-color .2s,color .2s,background .2s}
        button:active{transform:scale(.97)}
        input:focus,select:focus,textarea:focus{border-color:${C.rose}!important;box-shadow:0 0 0 3px ${C.roseLight}}
        ...
        ::selection{background:${C.roseLight};color:${C.dark}}
      `}</style>
      <nav style={S.nav}>
        <div style={S.logo}>Tot<span style={{color:C.rose}}>Day</span>{role==="agency"&&<span style={{fontFamily:fb,fontSize:11,fontWeight:700,color:C.rose,background:C.roseLight,padding:"2px 8px",borderRadius:6,marginLeft:8,letterSpacing:"0.04em"}}>AGENCY</span>}</div>
        <div style={{display:"flex",gap:2,flexWrap:"wrap"}}>
          {role==="agency"?(
            <>
              {/* Bug #4: используем handleGoToAgency вместо setTab("agency") */}
              <button style={S.tab(tab==="agency")} onClick={handleGoToAgency}>Кабинет</button>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          {role==="agency"&&openWedding&&<span style={{fontSize:12,color:C.rose,fontWeight:700}}>{openWedding.couple}</span>}
          {role!=="agency"&&survey?.date&&<span style={{fontSize:11,color:C.rose,fontWeight:700}}>{new Date(survey.date).toLocaleDateString("ru",{day:"numeric",month:"short"})}</span>}
          <div style={{width:32,height:32,borderRadius:"50%",background:`linear-gradient(135deg,${C.violet},${C.rose})`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,cursor:"pointer",fontWeight:700,color:"#fff",fontFamily:font}} onClick={logout} title="Выйти">
            {user.name1?.[0]||"А"}
          </div>
        </div>
      </nav>
      ...
      {role==="agency"&&tab==="agency"&&(
        <AgencyDashboard weddings={agencyWeddings} setWeddings={setAgencyWeddings} onOpen={handleOpenWedding}/>
      )}
      {!(role==="agency"&&tab==="agency")&&(<>
        ...
        {/* Bug #1: format:"restaurant" вместо "medium" */}
